...
use crate::codegen::lox_value::{LoxValue, LoxValueType, gen_alloc_heap_lox_value};
...
fn gen_panic_fn<'a>(module: &Module<'a>, builder: &Builder) -> anyhow::Result<FunctionValue<'a>> {
    ...
    builder.build_call(
        exit,
        &[context.i32_type().const_int(0xFFFFFFFF, false).into()],
        "_",
    )?;
...
fn gen_lox_object(ctx: &'_ Context) -> StructType<'_> {
    ...
    let type_biggest = ctx.i64_type();
    ctx.struct_type(&[type_i8.into(), type_biggest.into()], true)
}

/// LoxClosure: { fn_ptr: ptr, env: ptr (ptr-to-array-of-LoxValue*), n_env: i64 }
fn gen_closure_struct(ctx: &'_ Context) -> StructType<'_> {
    let ptr = ctx.ptr_type(AddressSpace::default());
    ctx.struct_type(&[ptr.into(), ptr.into(), ctx.i64_type().into()], false)
}
...
fn gen_var_decl(id: &str, rval: &Node, ast: &Ast, state: &mut State) -> anyhow::Result<()> {
    ...
    // heap-allocate so the variable can be captured by closures
    let dst = gen_alloc_heap_lox_value(LoxValueType::Nil, state)?;
    state.builder.build_store(dst.ptr, src)?;
    get_current_env(state).insert(id.to_owned(), dst);
    Ok(())
}
...
fn gen_declaration(decl: &Node, ast: &Ast, state: &mut State) -> anyhow::Result<()> {
    match decl {
        ...
        Node::ClassDecl(name, base, methods) => {
            gen_class::gen_class_decl(name, base.as_deref(), methods, ast, state)
        }
...
struct State<'a> {
    ...
    vars: HashMap<CString, VariableStack<'a>>,

    panic_fn: FunctionValue<'a>,
    lox_value: StructType<'a>,
    closure_type: StructType<'a>,
    instance_type: StructType<'a>,
...
pub fn codegen(ast: ast::Ast, context: &'_ mut Context) -> anyhow::Result<Module<'_>> {
    ...
    let closure_type = gen_closure_struct(context);
    let instance_type = gen_instance_struct(context);
    let class_type = gen_class_struct(context);
    ...
    // generate OOP runtime helper functions
    gen_class::gen_runtime_helpers(&mut state)?;

    gen_begin_main(&mut state);
