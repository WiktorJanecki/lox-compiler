    ...
    match expr {
        Node::Assignment(maybe_obj, lhs, rhs) => {
            if let Some(obj_id) = maybe_obj {
                ...
                let is_inst = state.builder.build_int_compare(IntPredicate::EQ, tag, inst_tag, "is_inst")?;
                state.builder.build_conditional_branch(is_inst, b_ok, b_err)?;
                ...
                let field_cstr = state.builder.build_global_string_ptr(lhs, "sfn")?.as_pointer_value();

                // heap-copy the value so lox_set_field can store a stable pointer
                let lox_val_size = state.lox_value.size_of().unwrap();
                let val_copy = state.builder.build_call(
                ...
                state.builder.build_call(set_field_fn, &[inst_ptr.into(), field_cstr.into(), val_copy.into()], "_")?;
                return Ok(right);
            }
        ...
        Node::Get(obj_id, field_name) => {
            ...
            let inst_ptr = gen_load_ptr(&obj, state)?;
            let field_cstr = state.builder.build_global_string_ptr(field_name, "gfn")?.as_pointer_value();
            let get_field_fn = state.module.get_function("lox_get_field").unwrap();
            let field_ptr = state.builder.build_call(get_field_fn, &[inst_ptr.into(), field_cstr.into()], "field_ptr")?
                .try_as_basic_value().basic().unwrap().into_pointer_value();
            ...
            state.builder.build_conditional_branch(found, b_found, b_not_found)?;
        ...
        Node::Super(method_name) => {
            ...
            let cls_gep = state.builder.build_struct_gep(state.instance_type, inst_ptr, 0, "cls_gep")?;
            let class_ptr = state.builder.build_load(ptr_t, cls_gep, "class_ptr")?.into_pointer_value();
            let super_gep = state.builder.build_struct_gep(state.class_type, class_ptr, 1, "super_gep")?;
            let super_ptr = state.builder.build_load(ptr_t, super_gep, "super_ptr")?.into_pointer_value();

            let mn_cstr = state.builder.build_global_string_ptr(method_name, "smn")?.as_pointer_value();
            let get_method_fn = state.module.get_function("lox_get_method").unwrap();
            let method_lox_ptr = state.builder.build_call(get_method_fn, &[super_ptr.into(), mn_cstr.into()], "super_method")?
                .try_as_basic_value().basic().unwrap().into_pointer_value();

            let b_ok = gen_block("super_ok", state);
            let b_err = gen_block("super_err", state);
            let found = state.builder.build_is_not_null(method_lox_ptr, "super_found")?;
...
fn gen_plus<'a>(
    ...
    let cases = &[
        ...
        (LoxValueType::Instance.llvm_int(state.ctx), unsupported_block),
    ...
    let left_str = state.builder.build_load(ptr_t, left_union, "left_str")?.into_pointer_value();
    let right_str = state.builder.build_load(ptr_t, right_union, "right_str")?.into_pointer_value();
    let strlen_fn = state.module.get_function("strlen").unwrap();
    let left_len = state.builder.build_call(strlen_fn, &[left_str.into()], "left_len")?
        .try_as_basic_value().basic().unwrap().into_int_value();
    let right_len = state.builder.build_call(strlen_fn, &[right_str.into()], "right_len")?
        .try_as_basic_value().basic().unwrap().into_int_value();
    let total = state.builder.build_int_add(left_len, right_len, "total_len")?;
    let total_plus_one = state.builder.build_int_add(total, i64_t.const_int(1, false), "total_plus_one")?;
    let malloc_fn = state.module.get_function("malloc").unwrap();
    let buf = state.builder.build_call(malloc_fn, &[total_plus_one.into()], "concat_buf")?
        .try_as_basic_value().basic().unwrap().into_pointer_value();
    let strcpy_fn = state.module.get_function("strcpy").unwrap();
    let strcat_fn = state.module.get_function("strcat").unwrap();
    state.builder.build_call(strcpy_fn, &[buf.into(), left_str.into()], "_")?;
    state.builder.build_call(strcat_fn, &[buf.into(), right_str.into()], "_")?;
...
fn gen_eq<'a>(
    ...
    state.builder.position_at_end(b_str);
    {
        ...
        let cmp = state.builder.build_call(strcmp_fn, &[left_str.into(), right_str.into()], "strcmp_res")?
            .try_as_basic_value().basic().unwrap().into_int_value();
        let str_eq = state.builder.build_int_compare(IntPredicate::EQ, cmp, state.ctx.i32_type().const_zero(), "str_eq")?;
    ...
    {
        let i64_t = state.ctx.i64_type();
        let lhs = state.builder.build_load(i64_t, left_union, "l_ptr")?.into_int_value();
        let rhs = state.builder.build_load(i64_t, right_union, "r_ptr")?.into_int_value();
        let eq = state.builder.build_int_compare(IntPredicate::EQ, lhs, rhs, "ptr_eq")?;
...
fn gen_call<'a>(
    ...
    if let Node::Get(obj_id, method_name) = node {
        ...
        let method_name_cstr = state.builder.build_global_string_ptr(method_name, "mn")?.as_pointer_value();
        let get_method_fn = state.module.get_function("lox_get_method").unwrap();
        let method_lox_ptr = state.builder.build_call(get_method_fn, &[class_ptr.into(), method_name_cstr.into()], "method_lox")?
            .try_as_basic_value().basic().unwrap().into_pointer_value();

        let b_has_method = gen_block("has_method", state);
        let b_no_method = gen_block("no_method", state);
        let has_method = state.builder.build_is_not_null(method_lox_ptr, "has_method")?;
        state.builder.build_conditional_branch(has_method, b_has_method, b_no_method)?;
    ...
    let is_closure = state.builder.build_int_compare(IntPredicate::EQ, tag, closure_tag, "is_closure")?;
    ...
    state.builder.build_conditional_branch(is_closure, b_closure, b_type_check)?;
    state.builder.position_at_end(b_type_check);
    state.builder.build_conditional_branch(is_class, b_class, b_err)?;
    ...
    let ret_val = state.builder.build_load(state.lox_value, ret.ptr, "ret_val")?;
    ...
    let inst_val = state.builder.build_load(state.lox_value, inst.ptr, "inst_val")?;
...
pub fn call_closure_value<'a>(
    ...
    let closure_int = state.builder.build_load(i64_t, union_ptr, "closure_int")?.into_int_value();
    let closure_obj_ptr = state.builder.build_int_to_ptr(closure_int, ptr_t, "closure_ptr")?;

    let fn_ptr_field = state.builder.build_struct_gep(state.closure_type, closure_obj_ptr, 0, "fn_ptr_f")?;
    let fn_ptr = state.builder.build_load(ptr_t, fn_ptr_field, "fn_ptr")?.into_pointer_value();
    let env_arr_gep = state.builder.build_struct_gep(state.closure_type, closure_obj_ptr, 1, "env_arr_gep")?;
    let env_arr = state.builder.build_load(ptr_t, env_arr_gep, "env_arr")?.into_pointer_value();

    let mut call_args: Vec<inkwell::values::BasicMetadataValueEnum> = vec![env_arr.into()];
    for node_id in args {
        let arg = gen_expr(&ast.nodes[*node_id], ast, state)?;
        let loaded = state.builder.build_load(state.lox_value, arg.ptr, "arg")?.into();
